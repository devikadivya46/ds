...
export default function AddPatientWizard({ isOpen, onClose, onSave, initialPatient }: AddPatientWizardProps) {
  ...
  const [department, setDepartment] = React.useState<'Cardiology' | 'Neurology' | 'General' | 'Oncology' | 'Pediatrics' | 'Intensive Care'>('Cardiology');
  ...
  const handleNext = () => {
    ...
    if (step === 1) {
      ...
      if (!doctor.trim()) {
        setErrorMsg('Attending treating physician coordinate is required.');
        return;
      }
      if (!bed.trim()) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedRecord: Patient = {
      ...
      crossConsultant: crossConsultant || 'None',
      status: finalStatus,
        ...
        <div className="p-8 max-h-[55vh] overflow-y-auto space-y-6 custom-scrollbar">
          ...
          {step === 2 && (
            <div className="space-y-5 animate-fadeIn">
              ...
              {/* Doctor & Consultant Selection Row */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label htmlFor="wizard-doctor" className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">
                    Treating Doctor Clinician *
                  </label>
                  <select
                    id="wizard-doctor"
                    value={doctor}
                  ...
                  >
                    <option value="Dr. Sarah Chen">Dr. Sarah Chen (Chief Registrar)</option>
                    <option value="Dr. Ashok Kumar">Dr. Ashok Kumar (Oncology Lead)</option>
                    <option value="Dr. Marcus Wright">Dr. Marcus Wright (Neurology)</option>
                    <option value="Dr. Anthony Fauci">Dr. Anthony Fauci (Internal Med)</option>
                  </select>
                </div>

                <div className="space-y-1.5">
                  <label htmlFor="wizard-cross" className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">
                    Cross Consultant Specialty
                  </label>
                  <select
                    id="wizard-cross"
                    value={crossConsultant}
                    onChange={(e) => setCrossConsultant(e.target.value)}
                    className="w-full h-14 px-4 bg-[#f1f3f9] border border-white/85 rounded-xl text-slate-800 font-extrabold focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none shadow-nm-inset-small transition-all text-xs cursor-pointer"
                  >
                    <option value="None">None - Sole Coordinator</option>
                    <option value="Dr. Sarah Chen">Dr. Sarah Chen (Cardiology)</option>
                    <option value="Dr. Ashok Kumar">Dr. Ashok Kumar (Oncology)</option>
                    <option value="Dr. Marcus Wright">Dr. Marcus Wright (Neurology)</option>
                    <option value="Dr. Anthony Fauci">Dr. Anthony Fauci (Infectious)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label htmlFor="wizard-dept" className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">
                  ...
                  >
                    ...
                    <option value="General">General Medicine & Ward</option>
                    <option value="Oncology">Oncology Department</option>
                    <option value="Pediatrics">Pediatrics Department</option>
                    <option value="Intensive Care">Intensive Care Sector</option>
                  </select>
                </div>
